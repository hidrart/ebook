                ...
                <form method="POST" action="{{ url("/book/$book->id") }}" class="mt-8 flex flex-col items-start gap-6">
                    ...
                    <label class="block w-full">
                        ...
                        <textarea name="description"
                            class="rounded-md border-0 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full bg-gray-100 mt-2 min-h-[60vh]">{{ old('description', $book->description) }}</textarea>
                    </label>
                    <button type="submit"
                        class=" bg-indigo-200 py-2 px-4 text-indigo-700 rounded-lg cursor-pointer flex items-center space-x-2">
                        <span>
